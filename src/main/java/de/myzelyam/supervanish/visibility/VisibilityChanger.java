...
public class VisibilityChanger {
        ...
        try {
            ...
            if (plugin.getVanishStateMgr().isVanished(player.getUniqueId())) {
                ...
                plugin.log(Level.WARNING,
                        "Failed to hide " + player.getName() + " since that player is already invisible.");
                return;
            ...
            silent = e.isSilent();
            // state
            plugin.getVanishStateMgr().setVanishedState(player.getUniqueId(), player.getName(), true, hiderName);
            // metadata
            ...
            if (config.getBoolean("InvisibilityFeatures.DisableMobTarget")) {
                ...
                player.getWorld().getEntities().stream().filter(ent -> ent instanceof Creature)
                        .map(ent -> (Creature) ent).filter(mob -> mob.getTarget() != null)
    ...
    public void showPlayer(final Player player, final String showerName, boolean silent) {
        ...
        try {
            ...
            // state
            plugin.getVanishStateMgr().setVanishedState(player.getUniqueId(), player.getName(), false, showerName);
            // chat message
            if (showerName == null)
            ...
            if (config.getBoolean("InvisibilityFeatures.Fly.DisableOnReappear") && !player.hasPermission("sv.keepfly")
